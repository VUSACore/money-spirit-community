...
export async function getNextSacredStep(
  ...
  const lastActivity = "recently active";

  const prompt = `The member's money archetype is ${archetype}. Their life stage is ${lifeStage}. Their ritual streak is ${streak} weeks. They have completed ${completedCoursesCount} courses. Their most recent activity was ${lastActivity}. Write them a personalised 'Next Sacred Step' — a warm 2-sentence prompt that honours their archetype and life stage and encourages their next action on the Money Spirit platform.`;
...
export async function scoreArchetypeFromBehaviour(
  ...
  try {
    ...
    // Fetch forum categories via thread_replies → threads → forums
