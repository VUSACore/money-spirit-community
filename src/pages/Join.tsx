  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: '#005eb8' }}>
      ...
      <header className="py-6 px-6 flex items-center justify-center gap-2.5">
        <span style={{ fontFamily: 'var(--font-display)', fontSize: '22px', color: '#E0B040', fontWeight: 700 }}>Money Spirit</span>
      </header>
      ...
      <main className="flex-1 max-w-[860px] mx-auto px-6 pb-12">
        <section className="text-center pt-8 pb-14">
          <h1 style={{
            ...
            fontWeight: 300, color: '#FFFFFF', letterSpacing: '-0.03em', lineHeight: 1.1, marginBottom: '16px',
          }}>
            Join the Money Spirit Community
          </h1>
          <p style={{ fontFamily: 'var(--font-body)', fontSize: '16px', color: '#D8C896', maxWidth: '500px', margin: '0 auto' }}>
          ...
          <div className="ss-card flex flex-col">
            ...
            <ul className="space-y-3 mb-8 flex-1">
              {features.monthly.map((f) => (
                <li key={f} className="flex items-start gap-2.5" style={{ fontSize: '14px', fontFamily: 'var(--font-body)', color: '#F0E8D4' }}>
                  <Check className="h-4 w-4 mt-0.5 flex-shrink-0" style={{ color: '#E0B040' }} /> {f}
                </li>
              ))}
            </ul>
        ...
        <p className="text-center mt-3">
          <Link to="/ethics" style={{ fontSize: '12px', fontFamily: 'var(--font-body)', color: '#E0B040', textDecoration: 'underline', textUnderlineOffset: '2px' }}>Ethics &amp; Education Policy</Link>
        </p>
      </main>
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle style={{ fontFamily: 'var(--font-display)', fontSize: '22px', fontWeight: 400, color: '#FFFFFF' }}>
              {submitted ? "You're on the list ✨" : "Membership is launching soon"}
            </DialogTitle>
          </DialogHeader>
          {submitted ? (
            <p style={{ fontSize: '14px', fontFamily: 'var(--font-body)', color: '#D8C896' }}>We'll let you know the moment doors open. Thank you for your interest.</p>
          ) : (
            <form onSubmit={handleNotify} className="space-y-4">
              <p style={{ fontSize: '14px', fontFamily: 'var(--font-body)', color: '#D8C896' }}>Drop your email to be first to know.</p>
