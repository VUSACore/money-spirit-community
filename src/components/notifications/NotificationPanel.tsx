...
const NotificationPanel = ({ userId, onClose, onCountChange }: NotificationPanelProps) => {
  ...
  return (
    <div className="w-[360px] max-h-[480px] overflow-hidden flex flex-col" style={{
      ...
      boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.10), 0 24px 60px rgba(0,0,0,0.6)',
    }}>
      <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <h3 style={{ fontFamily: 'var(--font-body)', fontWeight: 500, fontSize: '15px', color: 'var(--text-1)' }}>Notifications</h3>
        <button onClick={handleMarkAllRead} style={{ fontFamily: 'var(--font-body)', fontSize: '12px', fontWeight: 500, color: 'var(--text-gold)' }} className="hover:underline">Mark all read</button>
      </div>
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          <div className="p-3 space-y-3">
            {[1, 2, 3].map((i) => (<div key={i} className="flex items-start gap-3 p-2"><Skeleton className="w-8 h-8 rounded-full shrink-0" style={{ background: 'rgba(255,255,255,0.06)' }} /><div className="flex-1 space-y-1.5"><Skeleton className="h-3.5 w-3/4" style={{ background: 'rgba(255,255,255,0.06)' }} /></div></div>))}
          </div>
        ) : notifications.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 px-4">
            <Sparkles size={32} style={{ color: 'var(--text-4)' }} className="mb-3" />
            <p style={{ fontFamily: 'var(--font-body)', fontSize: '14px', color: 'var(--text-2)' }}>You are all caught up</p>
            <p style={{ fontFamily: 'var(--font-body)', fontSize: '12px', color: 'var(--text-4)', marginTop: '4px' }}>Notifications will appear here</p>
          </div>
        ) : (
          <div>
            {notifications.map((notif, idx) => {
              ...
              return (
                <button key={notif.id} onClick={() => handleClick(notif)}
                  ...
                  style={{ borderBottom: idx < notifications.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none', borderRadius: '8px' }}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    {notif.title && <p style={{ fontSize: '13px', fontFamily: 'var(--font-body)', fontWeight: 500, color: 'var(--text-1)', lineHeight: 1.4 }}>{notif.title}</p>}
                    <p className="line-clamp-2" style={{ fontSize: '12px', fontFamily: 'var(--font-body)', color: 'var(--text-2)', lineHeight: 1.4 }}>{notif.message}</p>
                    <p style={{ fontSize: '11px', fontFamily: 'var(--font-body)', color: 'var(--text-4)', marginTop: '2px' }}>{formatDistanceToNow(new Date(notif.created_at), { addSuffix: true })}</p>
                  </div>
                  {!notif.read && <span className="w-1.5 h-1.5 rounded-full shrink-0 mt-1.5" style={{ background: 'var(--gold-bright)' }} />}
                </button>
              );
            })}
          </div>
